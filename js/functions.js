...
var janela = '<div class="window" id="janela_'+id_user+'" style="'+style+'">';
		janela += '<div class="header_window"><a href="#" class="close icon-cross" style="font-size: 14px;margin-top: 2px;"></a></span> <span class="name" style="text-transform:capitalize;">'+nome+'</span><span id="'+id_user+'" class="'+status+'"></span></div>';
		janela += '<div class="body"><div class="mensagens"><ul></ul></div>';
		janela += '<div class="send_message" id="'+id+'"><input type="text" name="mensagem" class="msg" id="'+id+'" /><div class="opt_chat" id="opt_chat'+id+'"><span class="icon-attachment"></span><span class="icon-sphere"></span><span class="icon-image"></span><span class="icon-music"></span><span class="icon-newspaper"></span><span class="icon-cart"></span><span class="icon-location2"></span><span class="icon-airplane"></span><span class="icon-cog"></span><span class="icon-lock"></span><span class="icon-mic"></span><span class="icon-film"></span><span class="icon-home3"></span></div></div></div></div>';
				...
				jQuery.each(retorno, function(i, msg){
					if(jQuery('#janela_'+msg.janela_de).length > 0){
						if(userOnline == msg.id_de){
							jQuery('#janela_'+msg.janela_de+' .mensagens ul').append('<li id="'+msg.id+'" class="eu"><p class="ue">'+msg.mensagem+'</p></li>');
						}else{
							jQuery('#janela_'+msg.janela_de+' .mensagens ul').append('<li id="'+msg.id+'"  class="au"><div class="imgSmall"><img src="fotos/'+msg.fotoUser+'" /></div><p class="au">'+msg.mensagem+'</p></li>');
						}
					}
				});
				[].reverse.call(jQuery('#janela_'+id_conversa+' .mensagens li')).appendTo(jQuery('#janela_'+id_conversa+' .mensagens ul'));
				jQuery('#janela_'+id_conversa+' .mensagens').animate({scrollTop: 1000000000000000000000000}, '500');
			}
		});
	}
			...
			jQuery.ajax({
				...
				success: function(retorno){
					if(retorno == 'ok'){
						...
						//alert("Ocurrio Un Error Enviar El mensaje");
					}
				}
			});
		}
			...
			success: function(retorno){
				...
				if(retorno.status == 'resultados' || retorno.status == 'vazio'){
						...
						jQuery.each(retorno.dados, function(i, msg){
							...
							if(!in_array(msg.janela_de, clicou)){
								if(jQuery('.mensagens ul li#'+msg.id).length == 0 && msg.janela_de > 0){
									if(userOnline == msg.id_de){
										jQuery('#janela_'+msg.janela_de+' .mensagens ul').append('<li class="eu" id="'+msg.id+'"><p class="ue">'+msg.mensagem+'</p></li>');
									}else{
										jQuery('#janela_'+msg.janela_de+' .mensagens ul').append('<li class="au" id="'+msg.id+'"><div class="imgSmall"><img src="fotos/'+msg.fotoUser+'" border="0"/></div><p class="au">'+msg.mensagem+'</p></li>');
									}
								}
							}
						});
						jQuery('.mensagens').animate({scrollTop: 1000000000000000000000000}, '500');
						console.log(clicou);
					}
					clicou = [];jQuery('#users_online ul').html('');
					jQuery.each(retorno.users, function(i, user){
						...
						incluir += '<a href="javascript:void(0);" id="'+userOnline+':'+user.id+'" class="comecar">'+user.nome.slice(0,13)+'...'+'</a>';
					...
					alert('Ficamos confusos, actualize la pagina');
				}
			},
			error: function(){
				clearInterval(t);
